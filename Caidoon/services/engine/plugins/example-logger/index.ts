import { IPlugin, PluginContext, PluginMetadata, SessionEvent } from '@ai-encounters/core';
...
export default class ExampleLoggerPlugin implements IPlugin {
  ...
  async initialize(context: PluginContext): Promise<void> {
    ...
    // Get config
    const logToFile = context.config.logToFile as boolean;
   ...
   */
  private async logEvent(eventName: string, data: any): Promise<void> {
    ...
    if (this.logFile) {
      try {
        ...
        this.context.logger.error('Failed to write to log file', error as Error);
      }
    }
  }
}
