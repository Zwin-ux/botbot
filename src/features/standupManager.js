...
class StandupManager {
  ...
  scheduleStandup(config) {
    ...
    try {
      ...
      console.log(
        `Scheduled standup for ${config.guildId} in channel ${config.channelId} at ${config.scheduledTime} ${config.timezone}`,
      );
    } catch (error) {
  ...
  async startStandupSession(configId) {
    try {
      ...
      if (!channel) {
        console.error(
          `Channel ${config.channelId} not found for standup ${configId}`,
        );
        return;
      }
      ...
      console.error(
        `Error starting standup session for config ${configId}:`,
        error,
      );
    }
  }
  ...
  async generateStandupSummary(sessionId) {
    try {
      ...
      const config = session
        ? await this.getStandupConfig(session.configId)
        : null;

      let summary = `# Standup Summary - ${session.date}\n\n`;

      // Identify blockers
      const blockers = responses.filter(
      ...
      console.error(
        `Error generating standup summary for session ${sessionId}:`,
        error,
      );
    ...
    if (customId.startsWith("standup_respond_")) {
      ...
      try {
        ...
        if (!session) {
          return interaction.reply({
            content: "This standup session no longer exists.",
            ephemeral: true,
          });
        }
        ...
        if (responses.length === 0) {
          return interaction.reply({
            content: "No responses for this standup session yet.",
            ephemeral: true,
          });
        }
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `${responses.length} team members have provided updates so far.`,
          );
        ...
        for (const response of displayResponses) {
          ...
          embed.addFields({
            name: response.userTag,
            value: fieldContent || "No details",
          });
        }

        if (responses.length > 10) {
          embed.setFooter({
            text: `+ ${responses.length - 10} more responses. View full summary for details.`,
          });
        }
        ...
        await interaction.reply({
          content: "There was an error retrieving the standup responses.",
          ephemeral: true,
        });
      }
      ...
      try {
        ...
        if (summary.length <= maxChunkSize) {
          ...
          await interaction.reply({
            content: "Here's the full standup summary:",
            ephemeral: true,
          });
        }
      } catch (error) {
        ...
        await interaction.reply({
          content: "There was an error generating the standup summary.",
          ephemeral: true,
        });
      }
    }
  ...
  async handleStandupModalSubmit(interaction) {
    ...
    if (customId.startsWith("standup_modal_")) {
      ...
      try {
        await this.saveStandupResponse(
          sessionId,
          interaction.user.id,
        ...
        await interaction.reply({
          content: "Thanks for your standup update! 🎉",
          ephemeral: true,
        });
        ...
        await interaction.reply({
          content:
            "There was an error saving your standup response. Please try again.",
          ephemeral: true,
        });
      }
    }
