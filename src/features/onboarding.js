...
class Onboarding {
  ...
  async start(guild) {
    ...
    if (!sent) {
      ...
      if (channel) {
        await channel.send({
          content: `<@${owner.id}>`,
          embeds: [this.getWelcomeEmbed(guild)],
        });
        await channel.send({
          content: `<@${owner.id}>`,
          components: [this.getSetupButtons()],
        });
      }
    }
  }
  ...
  getSetupButtons() {
    return new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("onboard_standup")
        .setLabel("Schedule Standup")
        .setStyle(ButtonStyle.Primary),
      new ButtonBuilder()
        .setCustomId("onboard_reminder")
        .setLabel("Create Reminder")
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
        .setCustomId("onboard_games")
        .setLabel("Enable Games")
    ...
    switch (interaction.customId) {
      case "onboard_standup":
        await interaction.reply({
          content:
            "Let’s schedule your first standup! Type `setup standup in #channel at 9:30am` or use the standup commands in any channel.",
          ephemeral: true,
        });
        break;
      case "onboard_reminder":
        await interaction.reply({
          content:
            "You can create a reminder by typing `remind me to [task] [time]` in any channel. Try it now!",
          ephemeral: true,
        });
        break;
      case "onboard_games":
        await interaction.reply({
          content:
            "Games are enabled by default! Type `games` in any channel to see what’s available.",
          ephemeral: true,
        });
        break;
      case "onboard_skip":
        await interaction.reply({
          content:
            "No problem! You can set up features at any time. Type `help` or `what can I do here?` for guidance.",
          ephemeral: true,
        });
        break;
      default:
      ...
      .addFields(
        {
          ...
          value: "Just type what you want to do. No special commands needed.",
        },
        {
          name: "Set a Reminder",
          value:
            'Type: "remind me to call John tomorrow at 3pm" or "remind me in 30 minutes to check the oven". Recurring reminders work too.',
        },
        {
          name: "To-Do List",
          value:
        ...
        },
        {
          name: "Games",
          value:
            'Start: "start trivia" or "start emoji race"\nJoin: "join"\nEnd: "end game" (moderators)',
        },
        {
          name: "Examples",
          value:
