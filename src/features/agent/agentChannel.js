...
class AgentChannel {
  ...
  async ensureAgentChannel(guild) {
    ...
    let channel =
      settings && guild.channels.cache.get(settings.agent_channel_id);
  ...
  async sendWelcome(channel, ownerId) {
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        "This private channel is for server owner and agent-admins. All actions are logged and auditable. Type `help` for commands.",
      )
      .setColor("#5865F2")
  ...
  async handleMessage(msg) {
    ...
    const isAdmin = (await this.agentManager.listAdmins(msg.guild.id)).some(
      (a) => a.user_id === msg.author.id,
    );
    if (!isOwner && !isAdmin) {
    ...
    // Log every command
    await this.agentManager.logAction(msg.guild.id, msg.author.id, "COMMAND", {
      content: msg.content,
    });
    ...
    switch (cmd.toLowerCase()) {
      case "help":
        await msg.reply(
          "Agent commands: help, status, addadmin @user, removeadmin @user, listadmins, auditlog, safemode [on/off]",
        );
        break;
      case "status":
        await msg.reply(
          `Agent channel: <#${channelId}>\nOwner: <@${settings.owner_id}>\nAdmins: ${(await this.agentManager.listAdmins(msg.guild.id)).map((a) => `<@${a.user_id}>`).join(", ") || "None"}`,
        );
        break;
      case "addadmin":
        ...
        await msg.channel.permissionOverwrites.edit(addId, {
          ViewChannel: true,
          SendMessages: true,
      ...
      case "listadmins":
        ...
        await msg.reply(
          "Agent-admins: " +
            (admins.map((a) => `<@${a.user_id}>`).join(", ") || "None"),
        );
        break;
      case "auditlog":
        ...
        await msg.reply(
          "Recent actions:\n" +
            logs
              .map((l) => `${l.timestamp}: <@${l.user_id}> ${l.action}`)
              .join("\n"),
        );
        break;
      case "safemode":
