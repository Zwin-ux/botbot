...
class RetroManager {
  ...
  scheduleRetrospective(config) {
    ...
    try {
      ...
      console.log(
        `Scheduled retrospective for ${config.guildId} in channel ${config.channelId} ${config.frequency} at ${config.scheduledTime} ${config.timezone}`,
      );
    } catch (error) {
    ...
    try {
      ...
      if (config.frequency.toLowerCase() === "biweekly") {
        ...
        if (lastRunDate) {
          ...
          if (diffDays < 13) {
            console.log(
              `Skipping biweekly retro ${configId}, only ${diffDays} days since last run`,
            );
            return;
          }
        }
      ...
      if (!channel) {
        console.error(
          `Channel ${config.channelId} not found for retrospective ${configId}`,
        );
        return;
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "⏰ Please respond by",
            value: `End of day (${config.timezone})`,
          },
          {
            name: "🔍 We'll reflect on",
            value:
      ...
      console.error(
        `Error starting retrospective session for config ${configId}:`,
        error,
      );
    }
  }
  ...
  async generateRetroSummary(sessionId) {
    try {
      ...
      const config = session
        ? await this.getRetroConfig(session.configId)
        : null;
      ...
      console.error(
        `Error generating retrospective summary for session ${sessionId}:`,
        error,
      );
  ...
  async handleRetroInteraction(interaction) {
    ...
    if (
      customId.startsWith("retro_respond_") ||
      customId.startsWith("retro_anon_")
    ) {
      ...
      try {
        ...
        if (!session) {
          return interaction.reply({
            content: "This retrospective session no longer exists.",
            ephemeral: true,
          });
        }
        ...
        if (responses.length === 0) {
          return interaction.reply({
            content: "No responses for this retrospective session yet.",
            ephemeral: true,
          });
        }
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `${responses.length} team members have provided feedback so far.`,
          );
        ...
        if (responses.length > 5) {
          embed.setFooter({
            text: `+ ${responses.length - 5} more responses. View full summary for details.`,
          });
        }
        ...
        await interaction.reply({
          content: "There was an error retrieving the retrospective responses.",
          ephemeral: true,
        });
      }
      ...
      try {
        ...
        if (summary.length <= maxChunkSize) {
          ...
          await interaction.reply({
            content: "Here's the full retrospective summary:",
            ephemeral: true,
          });
        }
      } catch (error) {
        ...
        await interaction.reply({
          content: "There was an error generating the retrospective summary.",
          ephemeral: true,
        });
      }
    }
  ...
  async handleRetroModalSubmit(interaction) {
    ...
    if (customId.startsWith("retro_modal_")) {
      ...
      try {
        ...
        await interaction.reply({
          content:
            "There was an error saving your retrospective feedback. Please try again.",
          ephemeral: true,
        });
      }
    }
