...
class RetroHandler {
  ...
  async showRetroHelp(message) {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '!retro setup <frequency> <channel> <day> <time>', value: 'Set up a recurring retrospective (weekly, biweekly, monthly)' },
        { name: '!retro list', value: 'List all active retrospectives' },
        { name: '!retro start', value: 'Start a new retrospective session manually' },
        { name: '!retro cancel <id>', value: 'Cancel an active retrospective' },
        { name: '!retro summary <id>', value: 'Get the summary of a completed retrospective' }
      )
      .setFooter({ text: 'Example: !retro setup weekly #team-channel friday 15:00' });
  ...
  async setupRetro(message, args) {
    if (args.length < 3) {
      return message.reply('Usage: !retro setup <frequency> <channel> <day> <time> [timezone]');
    }
    ...
    if (args[1].startsWith('<#') && args[1].endsWith('>')) {
      ...
      channel = message.guild.channels.cache.find(ch => ch.name === args[1].replace('#', ''));
    }
    ...
    try {
      ...
      await message.reply('There was an error setting up the retrospective. Please try again.');
    }
  }
  ...
  async listRetros(message) {
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`${retros.length} active retrospective(s) for this server:`);
      ...
      await message.reply('There was an error retrieving the retrospectives. Please try again.');
    }
  }
  ...
  async startRetro(message, args) {
    try {
      ...
      await message.reply('Retrospective session started! Check the configured channel for the prompt.');
      
    } catch (error) {
      console.error('Error starting retrospective:', error);
      await message.reply('There was an error starting the retrospective. Please try again.');
    }
  }
  ...
  async cancelRetro(message, args) {
    if (!args.length) {
      return message.reply('Please specify the retrospective session ID to cancel.');
    }
    ...
    try {
      ...
      if (message.author.id !== config.createdBy && !message.member.permissions.has('ADMINISTRATOR')) {
        return message.reply('You don\'t have permission to cancel this retrospective.');
      }
      ...
      await message.reply(`Retrospective session #${sessionId} has been cancelled.`);
      
    } catch (error) {
      console.error('Error cancelling retrospective:', error);
      await message.reply('There was an error cancelling the retrospective. Please try again.');
    }
  }
  ...
  async getRetroSummary(message, args) {
    if (!args.length) {
      return message.reply('Please specify the retrospective session ID to get the summary for.');
    }
    ...
    try {
      ...
      await message.reply('There was an error retrieving the retrospective summary. Please try again.');
    }
  }
}
