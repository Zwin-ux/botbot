...
class StandupHandler {
  ...
  async showStandupHelp(message) {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: "!standup setup <channel> <time> [timezone]",
          value: "Set up a daily standup in the specified channel",
        },
        {
          name: "!standup list",
          value: "List all active standups in this server",
        },
        {
          name: "!standup start [id]",
          value: "Manually start a standup session",
        },
        {
          name: "!standup summary <id>",
          value: "Get a summary of standup responses",
        },
      )
      .setFooter({
        text: "Example: !standup setup #team-channel 09:30 America/Los_Angeles",
      });
  ...
  async setupStandup(message, args) {
    ...
    try {
      ...
      await message.reply(
        "There was an error setting up the standup. Please try again.",
      );
    }
  }
  ...
  async listStandups(message) {
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `${standups.length} active standup(s) for this server:`,
        );
      ...
      await message.reply(
        "There was an error retrieving the standups. Please try again.",
      );
    }
  }
  ...
  async startStandup(message, args) {
    try {
      ...
      await message.reply(
        "Standup session started! Check the configured channel for the prompt.",
      );
    } catch (error) {
      console.error("Error starting standup:", error);
      await message.reply(
        "There was an error starting the standup. Please try again.",
      );
    }
  }
  ...
  async getStandupSummary(message, args) {
    if (!args.length) {
      return message.reply(
        "Please specify the standup session ID to get the summary for.",
      );
    }
    ...
    try {
      const summary =
        await this.standupManager.generateStandupSummary(sessionId);
      ...
      await message.reply(
        "There was an error retrieving the standup summary. Please try again.",
      );
    }
  }
}
