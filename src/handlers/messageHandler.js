...
class MessageHandler {
  ...
  async handleMessage(msg) {
    ...
    if (msg.channel.type === "DM" || msg.guild) {
      ...
      try {
        ...
        if (
          lowerContent.match(
            /^(hi|hello|hey|ping|yo|sup|'?sup|are you there|bot\??)/i,
          )
        ) {
          return msg.reply(
            "Hi there! I'm here and ready to help with your reminders, standups, and retrospectives. What can I do for you today?",
          );
        }

        // Help command
        if (
          lowerContent.match(/^(help|what can you do|commands|support)\??/i)
        ) {
          return this.showHelp(msg);
        }

        // Handle standup commands
        const standupMatch = lowerContent.match(
          /(?:standup|daily)(?:\s+(setup|start|list|summary|help))?/i,
        );
        if (standupMatch) {
          if (this.standupHandler) {
            const subCommand = standupMatch[1] || "";
            const args = content
              .replace(/\b(?:standup|daily)\s*/i, "")
              .trim()
              .split(/\s+/);
            await this.standupHandler.processStandupCommand(msg, [
              subCommand,
              ...args,
            ]);
            return;
        ...
        const retroMatch = lowerContent.match(
          /(?:retro|retrospective)(?:\s+(setup|start|list|summary|help))?/i,
        );
        if (retroMatch) {
          if (this.retroHandler) {
            const subCommand = retroMatch[1] || "";
            const args = content
              .replace(/\b(?:retro|retrospective)\s*/i, "")
              .trim()
              .split(/\s+/);
            await this.retroHandler.processRetroCommand(msg, [
              subCommand,
              ...args,
            ]);
            return;
        ...
        const catHandler = new categoryHandler(
          this.client,
        ...
        if (this.guildHandler) {
          const handledByGuild = await this.guildHandler.handleMessage(
            msg,
            content,
          );
        ...
        const reminderListMatch = lowerContent.match(
          /(?:show|list|what.?s|my|view|see|check|display|get|what are (?:my|the)|do i have any|show me|tell me about)\s+(?:reminders?|todos?|tasks?|list|upcoming|pending|due|stuff|things|assignments)/i,
        );
        if (reminderListMatch) {
          // Check for category filter
          let categoryId = null;
          const categoryMatch = content.match(
            /in\s+(?:category|tag)\s+([^\s]+)/i,
          );
          if (categoryMatch) {
            const categoryEmoji = categoryMatch[1].trim();
            const category =
              await this.categoryManager.getCategoryByEmoji(categoryEmoji);
            if (category) {
          ...
          const timeFilterMatch = content
            .toLowerCase()
            .match(/(today|this week|overdue)/i);
          let timeFilter = "all";
          if (timeFilterMatch) {
        ...
        const doneMatch = content.match(
          /(?:done|complete|finish|completed|finished)\s+(\\d+)/i,
        );
        if (doneMatch) {
          ...
          try {
            ...
            return msg.reply({
              content:
                "I couldn’t find that reminder. Want to create a new one?",
              embeds: [embed],
              components: [row],
            });
          }
        ...
        if (
          content
            .toLowerCase()
            .match(
              /(remind|todo|reminder|task|remember|don'?t forget|need to|have to|should|must)/i,
            )
        ) {
  ...
  async showHelp(msg) {
    const helpEmbed = {
      ...
      description:
        "I understand natural language! Here's a breakdown of what I can do and some examples:",
      fields: [
        {
          name: "Reminders & Tasks",
          value:
            "**Setting Reminders/Tasks:**\n" +
  ...
  async handleIncompleteReminder(msg, task) {
    ...
    await msg.reply(
      `Got it! When would you like to be reminded to "${task}"? (e.g., "in 2 hours", "tomorrow at 3pm")`,
    );
  }
  ...
  async handleTimeResponse(msg, timeText) {
    ...
    if (!dueTime) {
      await msg.reply(
        "I couldn't understand that time. Could you try again? (e.g., 'in 2 hours', 'tomorrow at 3pm')",
      );
      return true;
    }
    ...
    let message =
      "Thanks! Would you like to add this to a specific category?\n\n";
  ...
  async handleCategoryResponse(msg, response) {
    ...
    if (response.toLowerCase().match(/^(none|skip|no|0)$/)) {
      ...
      response.match(
        /^create (\p{Emoji_Presentation}|\p{Extended_Pictographic}) (.+)$/iu,
      )
    ) {
      ...
      try {
        categoryId = await this.categoryManager.createCategory(
          name,
          emoji,
          `Created by ${msg.author.tag}`,
        );
        ...
        await msg.reply(
          "Sorry, I had trouble creating that category. I'll continue without a category.",
        );
      }
      ...
      if (categoryIndex >= 0 && categoryIndex < categories.length) {
        ...
        await msg.reply(
          `That's not a valid category number. I'll continue without a category.`,
        );
      }
    } else if (
      response.match(/(\p{Emoji_Presentation}|\p{Extended_Pictographic})/iu)
    ) {
      // User provided an emoji
      const matches = response.match(
        /(\p{Emoji_Presentation}|\p{Extended_Pictographic})/iu,
      );
      const emoji = matches[1];
      ...
      if (category) {
        ...
        await msg.reply(
          `I don't recognize a category with the emoji ${emoji}. I'll continue without a category.`,
        );
      }
    } else {
      await msg.reply(
        `I didn't understand that choice. I'll continue without a category.`,
      );
    }
  ...
  createReminderEmbed(reminder) {
    const embed = {
      color: 0x0099ff,
      title: "Reminder Set!",
      fields: [{ name: "Task", value: reminder.content, inline: true }],
      footer: {
        text: `ID: ${reminder.id}`,
      },
      timestamp: new Date(),
  ...
  async showRemindersSummary(msg, userId, options = {}) {
    try {
      const reminders = await this.reminderManager.getUserReminders(
        userId,
        options,
      );
      ...
      if (reminders.length === 0) {
        ...
        if (options.categoryId) {
          const category = await this.categoryManager.getCategoryById(
            options.categoryId,
          );
          if (category) {
      ...
      for (const reminder of reminders) {
        ...
        if (options.sortByPriority && reminder.priority !== undefined) {
          const prioritySymbol =
            reminder.priority > 0 ? "⬆️" : reminder.priority < 0 ? "⬇️" : "↔️";
      ...
      if (!options.timeFilter || options.timeFilter === "all") {
        ...
        const overdue = reminders.filter(
          (r) => r.dueTime && r.dueTime < now,
        ).length;

        if (today > 0 || overdue > 0) {
          let quickFilters = "\n\n**Quick Filters:**\n";
          if (today > 0)
            quickFilters += `- Show today's reminders (${today})\n`;
          if (overdue > 0)
            quickFilters += `- Show overdue reminders (${overdue})\n`;
      ...
      return msg.reply(
        "Sorry, I couldn't retrieve your reminders. Please try again later.",
      );
    }
  }
}
