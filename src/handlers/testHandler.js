...
class TestHandler {
    ...
    this.testPhrases = [
      {
        category: "Wake Words",
        phrase: "hey bot",
        description: "Basic wake word",
      },
      {
        category: "Wake Words",
        phrase: "okay bot",
        description: "Alternate wake word",
      },
      {
        category: "Wake Words",
        phrase: "yo bot",
        description: "Casual wake word",
      },
      {
        category: "Wake Words",
        phrase: "bot",
        description: "Minimal wake word",
      },
      {
        category: "Wake Words",
        phrase: "botbot",
        description: "Double wake word",
      },
      {
        category: "Wake Words",
        phrase: "hey botbot",
        description: "Combined wake word",
      },

      {
        category: "Intents",
        phrase: "hey bot, what can you do?",
        description: "Help intent",
      },
      {
        category: "Intents",
        phrase: "hey bot, remind me to check the server in 10 minutes",
        description: "Reminder intent",
      },
      {
        category: "Intents",
        phrase: "bot, I need help with something",
        description: "Help/blocked intent",
      },
      {
        category: "Intents",
        phrase: "yo bot, I'm stuck with this issue",
        description: "Blocked intent",
      },
      {
        category: "Intents",
        phrase: "hey bot, start a meeting",
        description: "Meeting intent",
      },
      {
        category: "Intents",
        phrase: "bot, let's play a game",
        description: "Game intent",
      },

      {
        category: "Edge Cases",
        phrase: "HEY BOT",
        description: "All caps wake word",
      },
      {
        category: "Edge Cases",
        phrase: "hey bot!",
        description: "Wake word with punctuation",
      },
      {
        category: "Edge Cases",
        phrase: "hey   bot",
        description: "Wake word with extra spaces",
      },
    ];
  }
  ...
  async handleMessage(message) {
    ...
    if (
      content === "bot test mode" ||
      content === "test bot nlp" ||
      content === "nlp test"
    ) {
  ...
  async runTest(message, index) {
    ...
    await message.reply(
      `**Running Test #${index + 1}:** \`${test.phrase}\` (${test.category} - ${test.description})`,
    );
      ...
      reply: async (content) => {
        if (typeof content === "object" && content.embeds) {
          return message.reply(
            `**Bot Response:** [Embed] ${content.embeds[0]?.description || "No description"}`,
          );
        }
    ...
    try {
      ...
      if (!handled) {
        await message.reply(
          `**Test Result:** The message was NOT handled by the natural language processor. This may indicate an issue with wake word detection or intent recognition.`,
        );
      }
    } catch (error) {
  ...
  async runCategoryTests(message, categoryName) {
    ...
    await message.reply(
      `**Running all ${categoryTests.length} tests in category:** ${categoryName}`,
    );
