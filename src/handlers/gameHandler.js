...
class RateLimiter {
  constructor(points, duration, execEvenly = false) {
    this.points = points;
    this.duration = duration;
    this.execEvenly = execEvenly;
    this.tokens = new Map();
  }
  ...
  take(key) {
    ...
    if (now > tokenData.resetTime) {
      tokenData.count = this.points;
      tokenData.resetTime = now + this.duration;
    }
...
class GameHandler {
  constructor(client, db) {
    ...
    this.rateLimiters = {
      gameStart: new RateLimiter(
        RATE_LIMIT.GAME_START.points,
        RATE_LIMIT.GAME_START.duration,
        true,
      ),
      gameCommand: new RateLimiter(
        RATE_LIMIT.GAME_COMMAND.points,
        RATE_LIMIT.GAME_COMMAND.duration,
        true,
      ),
    };
  }
      ...
      if (type === "gameCommand") {
        ...
        if (!commandLimit) {
          return {
            rateLimited: true,
            message:
              "You're using game commands too quickly. Please slow down.",
          };
        }
      }
  ...
  async validateGameStart(msg, gameType) {
    ...
    const now = Date.now();
    const channelCooldown = this.cooldowns.get(msg.channel.id) || {
      lastGameTime: 0,
    };
    ...
    if (this.gameManager.activeGames.has(msg.channel.id)) {
      return {
        valid: false,
        error:
          'There is already an active game in this channel. Type "end game" to end it first.',
      };
    }
  ...
  async handleJoinGame(msg) {
    ...
    if (!game) {
      const embed = createGameEmbed(null, {
        ...
        description:
          "There's no active game in this channel. Start one with `start game`!",
    ...
    try {
      ...
      const playerListEmbed = createPlayerListEmbed(players, "Player Joined");
  ...
  async showGameHelp(msg) {
      ...
      fields: [
        {
          name: "Emoji Race",
          value:
            "`start emoji race` - Race to be the first to react with the correct emoji!",
          inline: true,
        },
        {
          name: "Story Builder",
          value:
            "`start story` - Collaborate to build a story one sentence at a time!",
          inline: true,
        },
        {
          name: "Who Said It?",
    ...
    helpEmbed.description =
      'Start any game by typing something like "start emoji race" or "start story".';
    helpEmbed.fields = helpEmbed.fields.map((f) => ({
      ...f,
      value: f.value.replace(/`!?(start|join|end|games)[^`]*`/g, (match) =>
        match.replace(/!/g, ""),
      ),
    }));
        ...
        switch (gameType) {
          ...
          default:
            const embed = createGameEmbed(null, {
              ...
              description:
                "I couldn't find that game. Here are the available games:",
              color: COLORS.WARNING,
              emoji: EMOJIS.ERROR,
            });
    ...
    try {
      ...
      const joinEmbed = createGameEmbed("emoji-race", {
        ...
        fields: [
          {
            name: "How to Play",
            value: "Be the first to type the emoji sequence correctly!",
          },
    ...
    try {
      ...
      const joinEmbed = createGameEmbed("story-builder", {
        ...
        fields: [
          {
            name: "How to Play",
            value: "Take turns adding to the story, one sentence at a time!",
          },
    ...
    try {
      ...
      const joinEmbed = createGameEmbed("who-said-it", {
        ...
        fields: [
          {
            name: "How to Play",
            value: "Guess which server member said the quote!",
          },
