...
class EnhancedParserExtended {
  constructor() {
    this.timePatterns = [
      ...
      {
        regex:
          /(?:on\s+)?(next\s+)?(monday|tuesday|wednesday|thursday|friday|saturday|sunday)/i,
        handler: (match) => {
      ...
      {
        regex:
          /(?:on\s+)?(?:the\s+)?(\d{1,2})(?:st|nd|rd|th)?\s+(?:of\s+)?(january|february|march|april|may|june|july|august|september|october|november|december)/i,
        handler: (match) => {
      ...
      {
        regex:
          /this\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday|weekend)/i,
        handler: (match) => {
    ...
    this.taskPatterns = [
      ...
      {
        regex:
          /remind\s+(?:me|us|everyone|the team|#\w+)?\s*(?:to\s*)?(.*?)(?:\s+(?:at|on|in|by|tomorrow|next|tonight|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday|noon|midnight|morning|afternoon|evening|this|every|daily|weekly)|$)/i,
        handler: (match) => match[1].trim(),
      },
      // Todo format
      {
        regex:
          /(?:todo|task|add|create)\s+(.*?)(?:\s+(?:at|on|in|by|tomorrow|next|tonight|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday|noon|midnight|morning|afternoon|evening|this|every|daily|weekly)|$)/i,
        handler: (match) => match[1].trim(),
      },
      // Need to / Have to format
      {
        regex:
          /(?:need to|have to|must|should)\s+(.*?)(?:\s+(?:at|on|in|by|tomorrow|next|tonight|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday|noon|midnight|morning|afternoon|evening|this|every|daily|weekly)|$)/i,
        handler: (match) => match[1].trim(),
      },
      // Task at time format
      {
      ...
      {
        regex:
          /(?:don'?t forget|remember) to\s+(.*?)(?:\s+(?:at|on|in|by|tomorrow|next|tonight|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday|noon|midnight|morning|afternoon|evening|this|every|daily|weekly)|$)/i,
        handler: (match) => match[1].trim(),
      },
      // "I need a reminder to" format
      {
        regex:
          /(?:i need|set|create) a reminder to\s+(.*?)(?:\s+(?:at|on|in|by|tomorrow|next|tonight|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday|noon|midnight|morning|afternoon|evening|this|every|daily|weekly)|$)/i,
        handler: (match) => match[1].trim(),
      },
    ];
    ...
    this.recurringPatterns = [
      {
        regex:
          /(?:every|each)\s+(day|morning|evening|night|week|monday|tuesday|wednesday|thursday|friday|saturday|sunday|weekday|weekend)(?:\s+at\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?)?/i,
        handler: (match) => {
      ...
      {
        regex:
          /(?:daily|weekly|monthly)(?:\s+at\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?)?/i,
        handler: (match) => {
          const [_, hours, minutes, ampm] = match;
          const frequency = match[0].startsWith("daily")
            ? "day"
            : match[0].startsWith("weekly")
              ? "week"
              : "month";
    ...
    this.targetPatterns = [
      {
        ...
        handler: (match) => {
          ...
          if (
            ...
            return { type: "group", target: "everyone" };
          if (target === "the channel")
            return { type: "channel", target: "current" };
      ...
      },
      {
        ...
        handler: (match) => {
          ...
          if (target.startsWith("@"))
            return { type: "user", target: target.substring(1) };
          if (target.startsWith("#"))
            return { type: "channel", target: target.substring(1) };
          return { type: "user", self: true }; // Default to self
        },
      },
    ];
  ...
  extractTask(text) {
    ...
    for (const { regex } of this.timePatterns) {
      ...
      if (match) {
        ...
        response.task = text
          .replace(new RegExp(`\\s*${timeMatch}\\s*`, "i"), " ")
          .trim();
        break;
      }
    }
