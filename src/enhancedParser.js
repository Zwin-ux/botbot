  ...
  constructor() {
    ...
    this.timePatternsByLanguage = {
      ...
      es: [
        ...
        {
          regex:
            /en\s+(\d+)\s+(minuto|minutos|hora|horas|día|días|semana|semanas)/i,
          handler: (match) => {
            const [_, amount, unit] = match;
            const numAmount = parseInt(amount);
            const now = new Date();
    ...
    this.taskPatterns = [
      ...
      {
        regex:
          /remind\s+(?:me\s*to\s*)?(.*?)\s*(?:at|on|in|tomorrow|next|tonight|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday|$)/i,
        handler: (match) => match[1].trim(),
      },
      // "todo [task] [time]"
      {
        regex:
          /todo\s+(.*?)\s*(?:at|on|in|tomorrow|next|tonight|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday|$)/i,
        handler: (match) => match[1].trim(),
      },
      // "[task] at [time]"
      {
