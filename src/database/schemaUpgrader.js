        ...
        try {
          ...
          this.db.get("PRAGMA table_info(reminders)", (err, row) => {
            ...
            if (!row || !row.find(col => col.name === 'categoryId')) {
              ...
              this.db.run(`ALTER TABLE reminders ADD COLUMN categoryId INTEGER`);
              this.db.run(`ALTER TABLE reminders ADD COLUMN priority INTEGER DEFAULT 0`);
              ...
              const defaultCategories = [
                { name: 'General', emoji: '📝', description: 'General reminders' },
                { name: 'Personal', emoji: '👤', description: 'Personal tasks and reminders' },
                { name: 'Work', emoji: '💼', description: 'Work-related tasks' },
                { name: 'Urgent', emoji: '🚨', description: 'High priority tasks' },
                { name: 'Project', emoji: '🚀', description: 'Project-related tasks' }
              ];
              
              const stmt = this.db.prepare('INSERT INTO categories (name, emoji, description) VALUES (?, ?, ?)');
