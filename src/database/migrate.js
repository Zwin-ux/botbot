...
class MigrationRunner {
  ...
  async runMigrations() {
    ...
    const lastBatch = (await this.get(
    ...
    for (const file of files) {
      if (!completed.has(file)) {
        ...
        try {
          ...
          await this.run("INSERT INTO migrations (name, batch) VALUES (?, ?)", [
            file,
            nextBatch,
          ]);
  ...
  async rollback(steps = 1) {
    ...
    const latestBatch = (await this.get(
      "SELECT MAX(batch) as maxBatch FROM migrations",
    )) || { maxBatch: 0 };
    ...
    console.log(
      `Rolling back ${migrationsToRollback.length} migration(s) from batch ${latestBatch.maxBatch}:`,
    );
