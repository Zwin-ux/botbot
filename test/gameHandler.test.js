...
describe('GameHandler', () => {
  ...
  beforeEach(() => {
    ...
    // Reset test data
    testChannel = { id: 'test-channel', send: async () => ({ id: 'test-message' }) };
  ...
  describe('Game Commands', () => {
    ...
    it('should only allow game starter or admin to end game', async () => {
      // Start a game
      const startMessage = new MockMessage('start emoji race', testUser, testChannel);
      ...
      expect(endMessage.replies[0]).to.include('Only the game starter or an admin');
